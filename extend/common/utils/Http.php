...
class Http {
	 ...
	 * 获取前台HTTP HOST
	 */
	public static function getFrontHost($http=true,$appid='') {
		$host=self::getDomain();
		$path = explode ( '.', $host );
		if (Math::isInteger ( $path[0] )) {
			array_shift ( $path );
			$host = implode ( '.', $path );
		}
		if($appid){
		...
		return $http?'http://'.$host:$host;
	}
	/**
	 * 获取私有链接
	 */
	public static function getPrivateHost($cid,$http=true) {
		$host=self::getDomain();
		$path = explode ( '.', $host );
		
		if(!Math::isInteger($path[0])){
			if ('wap' == $path[0]||'api'==$path[0]) {
				array_shift($path);
				$host= implode('.', $path);
			}elseif(is_string($path[0])&&preg_match('/^wx[A-Za-z0-9]{16}$/',$path[0])){  // 识别为appid
				array_shift($path);
				$host= implode('.', $path);
			}
			$host= $cid . '.' . $host;
	 ...
	 * 获取私有链接
	 */
	public static function isPrivateHost() {
		$host=self::getDomain();
		$path = explode ( '.', $host );
		if (Math::isInteger ( $path[0] )) {
			return true;
